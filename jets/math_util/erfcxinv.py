...
def CreateData(y_range=(0.,2.),npoints = 1000000,filename='erfcxinv.h5'):
    ...
    x = np.flip(np.linspace(*y_range,npoints)) # function monotonically decreases, we want f(x) to increase with index, thus perform flip
    y = spec.erfcx(x)
...
def Evaluate(x,filename='erfcxinv.h5',y_range=(0.,2.),npoints=1000000):
    
    if(not pathlib.Path(filename).exists()):
        CreateData(filename=filename,y_range=y_range,npoints=npoints)
...
def EvaluatePrecise(x,filename='erfcxinv.h5', tolerance=0.001, nsteps=100, y_range=(-3.,50.), npoints=100, n_recursion=0):    
    max_recursion = 10
    if(n_recursion >= max_recursion): return -1.
    ...
    for i,x_val in enumerate(x):
        ...
        while(error > tolerance):
            ...
            y_val = EvaluatePrecise(np.array([x_val]),tmp_file,tolerance,nsteps,y_range=(yf,yi),npoints=npoints)
            x_test = spec.erfcx(y_val)
