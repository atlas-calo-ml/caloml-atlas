...
def TrainNetwork(regressor, modelfile, x_train, y_train, x_valid, y_valid, sample_weight, callbacks, overwriteModel=False, saveModel=True):
    ...
    if(pathlib.Path(model_filename).exists() and not overwriteModel):
        ...
        initial_epoch = 0
        history_filename = model_filename.replace('.h5','.csv')
